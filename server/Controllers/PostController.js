...
// Modify the existing getTimelinePosts to support category filtering
export const getTimelinePostsWithCategory = async (req, res) => {
  const userId = req.params.id;
  ...
  try {
    ...
    let allFollowingPosts = followingPosts[0]?.followingPosts || [];
    
    // Apply category filter to following posts if specified
    if (category && category !== 'all') {
      allFollowingPosts = allFollowingPosts.filter(post => post.category === category);
    }

    const allPosts = currentUserPosts.concat(allFollowingPosts).sort((a, b) => {
      return b.createdAt - a.createdAt;
    });

    res.status(200).json(allPosts);
  } catch (error) {
    console.error("Error fetching timeline posts with category:", error);
    res.status(500).json(error);
  }
};